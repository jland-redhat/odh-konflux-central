...
5. **`must-gather`** — official RHOAI must-gather (`registry.redhat.io/rhoai/odh-must-gather-rhel9` + `rhoai-must-gather-tag`, default `v3.5.0`) for **`aigateway`** (includes MaaS CRs / Kuadrant TRLPs) and **`kserve`** (LLMInferenceService), plus plain OpenShift must-gather, under `ARTIFACT_DIR` (also `onError: continue`).
