...
5. **`must-gather`** — RHOAI must-gather (`quay.io/rhoai/odh-must-gather-rhel9:rhoai-3.6`) for **`aigateway`** (includes MaaS CRs / Kuadrant TRLPs) and **`kserve`** (LLMInferenceService), plus plain OpenShift must-gather, under `ARTIFACT_DIR` (also `onError: continue`).
