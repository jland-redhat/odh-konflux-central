...
5. **`must-gather`** — RHOAI must-gather (`quay.io/modh/must-gather`) for **`aigateway`** (includes MaaS CRs / Kuadrant TRLPs) and **`kserve`** (LLMInferenceService), plus plain OpenShift must-gather, under `ARTIFACT_DIR` (also `onError: continue`).
